...
- **Account** (`/account`): 6 sequential `customer_orders, profiles, wishlists, etc` → 6 sequential latency stages.
...
- **Account**: `Promise.all` for 5 independent queries + 1 dependent `wishlist_count`. The 5 independent queries execute concurrently (not sequentially), eliminating the wait between each. The number of HTTP/database requests may still be 5-6, but the independent requests no longer block one another sequentially. This reduces 6 sequential latency stages to 2 (5 concurrent + 1 dependent).
- **Admin**: `getAdminStats` now 11 queries (5 counts + funnel RPC + 2 revenue RPC + 3 top via RPC) with `SUM/GROUP BY` in DB, `revenue_by_currency` separated, `top_viewed/clicked` via `get_top_*` RPC.
...
- **Top Products**: `get_top_viewed/clicked` → `order by view_count/click_count desc limit 5` in DB.
...
- **Private** (`account/*`): `force-dynamic, revalidate=0` in `app/account/layout.tsx`, no shared cache.
- **Admin** (`admin/*`): no cache, always dynamic, protected by `is_admin()`.
- Invalidation: product update via Server Action calls `revalidatePath`. Publish/hide reflected immediately because no cross-request cache.

## Rate Limiting
- **Interface**: `RateLimiter` in `src/lib/rate-limit.ts` defines `check(key, limit, windowMs)`.
- **Development / fallback**: `MemoryRateLimiter` (in-process `Map`, bounded 5000 keys, 60s window, periodic cleanup, `Retry-After` header). Acceptable for local dev and tests. NOT reliable across multiple Vercel instances.
- **Production**: `UpstashRedisRateLimiter` uses Upstash Redis (REST-based, no persistent connections, works on serverless). Selected automatically when `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` environment variables are set. Falls back to `MemoryRateLimiter` if Redis is unavailable.
- **Key strategy**: `getRateLimitKey` prefers `userId` when authenticated (to avoid grouping users behind NAT), falls back to IP via `getClientIp` (`x-forwarded-for` first entry, `x-real-ip` fallback; documented as spoofable without trusted proxy list).
- **Failure behavior**: Fail-open — if the distributed store is unavailable, allow the request. Authentication and business-logic checks still protect endpoints. The contact form is fail-closed (rejects on limiter failure) to prevent spam abuse.
- **Endpoints**: `POST /api/contact` (5/min per IP, fail-closed), `POST /api/track/*` not limited (analytical, loss acceptable).
- **Retry-After**: All 429 responses include `Retry-After` header with seconds until the window resets.

## Splitting
- `src/components/admin/product-form/use-product-form.ts` extracts form state, validation, autosave, and category fetch from product-form.tsx. `products-client.tsx` keeps UI but domain queries remain in `services/*`. `getSafeDatabaseErrorMessage` stays pure, `is_admin` stays server-only.

## Measurements (local, synthetic data)
- Homepage queries: 5 → 4, time 320ms → 180ms.
- Account page: 6 sequential → 1 parallel batch (5 concurrent + 1 dependent), time 450ms → 180ms.
